...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Issue #14: track which Intelligence Mode is active
  const [activeNavId, setActiveNavId] = useState<string>('nav-assistant');
  const navItems = [
    {
      id: 'nav-assistant',
    ...
    <>
      ...
      >
        ...
        <div className="p-4 pb-3 border-b border-slate-800/60 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              {/* Issue #3: bump tagline from 10px → 12px */}
              <p className="text-[12px] text-slate-400 tracking-wide font-medium">
        ...
        <div className="px-3 py-1 space-y-0.5">
          {/* Issue #4: drop all-caps; issue #5: 10px → 11px */}
          <span className="px-2 text-[11px] font-semibold tracking-wide text-slate-400">
            Intelligence Modes
          </span>
          {/* Issues #11 & #14: badge inline next to label; active highlight */}
          {navItems.map((item) => {
            const Icon = item.icon;
            const isNavActive = item.id === activeNavId;
            return (
              <button
                key={item.id}
                id={item.id}
                ...
                className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-xs transition-colors group cursor-pointer ${
                  isNavActive
              ...
              >
                <Icon
                  className={`w-3.5 h-3.5 shrink-0 transition-colors ${
                    isNavActive ? 'text-emerald-400' : 'text-slate-400 group-hover:text-emerald-400'
                  }`}
                />
                <span>{item.label}</span>
                {/* Badge placed right after label — no gap to close */}
                <span
        ...
        <div className="flex-1 overflow-y-auto px-3 py-1 min-h-0">
          <div className="flex items-center justify-between px-2 pb-2">
            {/* Issue #5: 10px → 11px, drop all-caps */}
